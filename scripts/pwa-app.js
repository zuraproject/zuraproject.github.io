...
async function renderLibrary(filter = '') {
  ...
  filtered.sort((a, b) => b.created - a.created).forEach(item => {
    ...
    delBtn.addEventListener('click', async e => { e.stopPropagation(); await deleteMedia(item.id); await renderLibrary(); await loadRecent(); });
...
dismissBtn.addEventListener('click', () => { a2hsBanner.style.display = 'none'; localStorage.setItem('a2hsDismissed', 'true'); });
